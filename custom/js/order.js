...
$(document).ready(function() {
	...
	if(divRequest == 'add')  {
		...
		$("#createOrderForm").unbind('submit').bind('submit', function() {
			...
			if(areaName == "") {
				$("#areaName").after('<p class="text-danger"> El area es requerida </p>');
				$('#areaName').closest('.form-group').addClass('has-error');
			} else {
				$('#areaName').closest('.form-group').addClass('has-success');
			} // /else
			if(clientName == "") {
		    ...
		    if(productName[x].value == ''){	    		    	
		    	$("#"+productNameId+"").after('<p class="text-danger"> NOMBRE DE PRODUCTO REQUERIDO </p>');
	   	...
	   	for (var x = 0; x < quantity.length; x++) {       
		    ...
		    if(quantity[x].value == ''){	    	
		    	$("#"+quantityId+"").after('<p class="text-danger"> CANTIDAD ES REQUERIDA </p>');
	   	...
	   	for (var x = 0; x < quantity.length; x++) {       						
			...
			if(orderDate && clientName && clientContact ) {
				if(validateProduct == true && validateQuantity == true) {
					console.log('Entro a validateproduct y canditad');
					// create order button
	            	...
	            	'<a href="orders.php?o=add" class="btn btn-default" style="margin-left:10px;"> <i class="glyphicon glyphicon-plus-sign"></i> Agregar Nueva </a>'+
...
function discountFunc() {
	var discount = $("#discount").val();
 	...
 	var grandTotal;
 	if(totalAmount) { 	
 		grandTotal = Number($("#totalAmount").val()); //- Number($("#discount").val()); aLDO
 		grandTotal = grandTotal.toFixed(2);
